...
\begin{column}{0.5\textwidth}
    ...
    \visible<2->{\item{Review by someone who did not write the code (\alert{software design verification}).}}
    \visible<3->{\item{Iterative process}}
    \visible<4->{\item{Permanently saved (\alert{configuration change control})}}
    \end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
