...
\item{It is worth understanding/complying with NRC regulatory standards:}
\begin{itemize}
...
\item{\alert{Configuration status control} - changes are accounted for prior to being incorporated into the baseline \cite{add}}
\end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
\item{PyNE developers are a diverse bunch}:
   ...
   \item{Ability: varying software development backgrounds}
...
\begin{itemize}
\item{All developers access a software baseline, make changes, and request that the changes are incorporated into the baseline}
\item{Each change is analogous to a mini-waterfall method}
\end{itemize}

\centerline{\includegraphics[width=0.75\textwidth]{figures/agile.png}}
...
\item{Integrated into GitHub using Polyphemus \cite{polyphemus_2014}}
\end{itemize}
\end{frame}
...
Declaring modules \emph{QA compliant}:
\begin{enumerate}
...
\item{For modules with significant physics (e.g.\ transport, transmutation):}
    \begin{itemize}
    \item{benchmarking with experimental results}
